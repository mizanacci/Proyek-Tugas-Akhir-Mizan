...
float hitungRMS(SensorId id) {
  ...
  for (int i = 0; i < JUMLAH_SAMPEL_RMS; i++) {
    ...
    delayMicroseconds(INTERVAL_ADC_US);
  }
...
HasilSensor bacaSemuaSensor() {
  ...
  hasil.tegangan_sumber = (adc_dc_sumber / ADC_MAX_VALUE) * 3.3f * RASIO_PEMBAGI_SENSOR_DC;
#else
  ...
  // (185 mV/A untuk varian 5A). Asumsi referensi ADC 3.3V, ADC_MAX_VALUE
  // langkah penuh — TITIK INI PERLU DIVERIFIKASI ULANG bila memakai
  // kalibrasi regresi seperti ZMPT101B untuk akurasi lebih baik (opsional,
  // di luar cakupan Panduan Kalibrasi ZMPT101B yang sudah dibuat).
  const float referensi_mv = 3300.0f;
